...
# 提取自变量（X）和因变量（Y）
decide=input("普通坐标or对数坐标:")
decidecut=input("选择切片类型(前切片，后切片，部分切片)")

if decide=="普通坐标":
   X = data[['t(h)']]  # 替换为你的 Excel 表中自变量的列名
   Y = data['Pwf(MPa)']     # 替换为你的 Excel 表中因变量的列名
else:
   X = data[['lgt']]  # 替换为你的 Excel 表中自变量的列名
   Y = data['Pwf']     # 替换为你的 Excel 表中因变量的列名

if decidecut=="部分切片":
   cutpoint1,cutpoint2=input("切片范围(a,b):").split(",")
   X_slice = X.iloc[eval(cutpoint1):eval(cutpoint2)]  # 切片索引5到最后
   Y_slice = Y.iloc[eval(cutpoint1):eval(cutpoint2)]
elif decidecut=="前切片":
   cutpoint1=input("切片起始位置:")
   X_slice = X.iloc[eval(cutpoint1):]  # 切片索引5到最后
   Y_slice = Y.iloc[eval(cutpoint1):]
elif decidecut=="后切片":
   cutpoint2=input("切片终止位置:")
   X_slice = X.iloc[:eval(cutpoint2)]  # 切片索引5到最后
   Y_slice = Y.iloc[:eval(cutpoint2)]

# 使用 np.polyfit 拟合一个 5 次多项式（可以根据需求调整阶数）多项式拟合
'''degree = 5
coefficients = np.polyfit(X.values.flatten(), Y, degree)'''
# 生成插值模型
