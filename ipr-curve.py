...
if pr>pb:   #分段计算
    if pwf<=pb:
        ...
        jo=qo/(pr-pwf)
    for pwf_test in range(0,1+int(pr)):       #打点计算pwf对应的值
        if pwf_test<pb:
    ...
    for pwf_test in range(0,1+int(pr)):                   #打点计算曲线坐标
...
#转化为csv
ls_csv=[y_label,x_label]
with open('c:/Users/Jason Shen/Desktop/采油工程/iprdata.csv','a',newline='') as f:
    writer=csv.writer(f)
...
plt.grid(True)  # 显示网格
plt.savefig("c:/Users/Jason Shen/Desktop/采油工程/test2.png", dpi=300, bbox_inches='tight')
'''
